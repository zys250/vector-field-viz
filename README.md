...
当前项目也保留 PWA/service worker 作为浏览器增强能力，但主交付建议优先使用 Windows 可执行文件或 Android APK。应用内的网络/离线缓存状态不会常驻画布，只在“运行设置”面板的诊断区显示。
...
- 给 `public/` 补正式应用图标后再发布首个 release。
- GitHub release 中同时提供 Windows portable 包和 Android debug/release APK。
